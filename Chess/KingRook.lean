...
`native_decide` of `checkFile` in the eight `Chess.KingRookCover` modules
...
Captures of the rook are excluded from the policy: they leave two kings.
...
a color swap. `Chess.KingRookTheorems` decides `CheckmateReachable` for
a valid king-and-rook versus king position and produces a concrete mating
...
theorem progress_exists {s : KRState} (hall : checkAll = true)
    (hok : s.okB = true) :
    s.mateB = true ∨ s.deadB = true ∨ ∃ s1, Progress s s1 := by
...
theorem checkmateReachable_of_okB_of_not_dead (hall : checkAll = true)
    {s : KRState} (hok : s.okB = true) (hnd : s.deadB = false) :
